...
type CryptoEvidenceModalProps = {
  evidence: CryptoEvidence
  onClose: () => void
}

export default function CryptoEvidenceModal({ evidence, onClose }: CryptoEvidenceModalProps) {
  ...
  const handleArticleClick = () => {
    if (evidence.articleUrl) {
      window.open(evidence.articleUrl, "_blank", "noopener,noreferrer")
    }
  }
    ...
    >
      ...
      >
        <CardHeader className="relative border-b border-amber-900/20 bg-amber-100/50">
          ...
          <CardDescription className="font-mono text-amber-700">CRYPTO INVESTIGATION</CardDescription>
        </CardHeader>
